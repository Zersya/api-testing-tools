...
// isDesktop is no longer imported at top level - use checkIsDesktop() in onMounted
...
// Safe check for desktop mode that works during SSR
...
// Don't use immediate: true at top level due to SSR issues
...
// Initialize on mount
onMounted(async () => {
  isDesktopMode.value = checkIsDesktop();
  ...
  if (isDesktopMode.value) {
    ...
    await Promise.all([
      refreshWebMocks(),
      refreshWebCollections(),
      refreshWebWorkspaces(),
      refreshDefinitions()
    ]);
  }
});
...
<template>
  <div class="flex flex-col h-screen overflow-hidden">
    <!-- Header -->
    <AppHeader title="Mock Services" :environments="safeEnvironments" :active-environment="activeEnvironment"
      @open-settings="openSettings" @export-open-a-p-i="exportOpenAPI" @import-open-a-p-i="openImportModal"
      @activate-environment="activateEnvironment" />

    <div class="flex flex-1 overflow-hidden">
      <!-- Sidebar -->
      <AppSidebar :collections="safeCollections" :mocks="safeMocks" :selected-mock-id="selectedMock?.id"
        :workspaces="safeWorkspaces" :selected-workspace-id="selectedWorkspaceId"
        :refresh-trigger="definitionsRefreshTrigger" @select-mock="handleSelectMock"
        @select-request="handleSelectRequest" @create-mock="goToCreate" @create-resource="showResourceModal = true"
        @create-collection="openCreateCollection" @create-request="openCreateRequest" @create-folder="openCreateFolder"
        @create-project="openCreateProject" @create-workspace="openCreateWorkspace"
        @rename-workspace="openRenameWorkspace" @rename-project="openRenameProject"
        @delete-project="confirmDeleteProject" @edit-collection="openEditCollection"
        @rename-collection="openRenameCollection" @delete-collection="confirmDeleteCollection"
        @delete-group="confirmDeleteGroup" @delete-folder="confirmDeleteFolder" @rename-folder="openRenameFolder"
        @delete-request="confirmDeleteRequest" @restore-request="handleRestoreRequest" @compare="handleCompareResponses"
        @view-definition-docs="handleViewDefinitionDocs" @generate-definition-mocks="handleGenerateDefinitionMocks"
        @reimport-definition="handleReimportDefinition" @reorder-folders="handleReorderFolders"
        @reorder-requests="handleReorderRequests" @select-workspace="selectedWorkspaceId = $event"
        @import-complete="definitionsRefreshTrigger++" />

      <!-- Main Content -->
      <main class="flex flex-col flex-1 overflow-hidden bg-bg-primary">
        <!-- Empty State -->
        <div v-if="!selectedMock && !selectedRequest && openTabs?.length === 0"
          ...
          <p class="text-text-secondary mb-6 max-w-[340px]">Choose an endpoint from the sidebar to view details, test
            it, and generate code snippets</p>
        ...
        <div v-if="openTabs?.length > 0" class="flex flex-col flex-1 overflow-hidden">
          <!-- Tabs -->
          <RequestTabs :open-tabs="openTabs" :active-tab-key="activeTabKey" @select-tab="handleSelectTab"
            @close-tab="handleCloseTab" @new-tab="handleNewTab" @reorder-tabs="handleReorderTabs" />

          <!-- Request Builder -->
          <RequestBuilder v-if="selectedRequest && activeTabKey" :request="selectedRequest"
            :workspace-id="currentWorkspaceId" :environment-id="activeEnvironment?.id" :project-id="currentProjectId"
            @save-request="handleSaveRequest" @save-as-request="handleSaveAsRequest"
          ...
          <div class="flex items-center gap-3 p-3 px-4 bg-bg-secondary border border-border-default rounded-lg">
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                @click="copyPath(selectedMock)" title="Copy URL">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                  stroke-linecap="round" stroke-linejoin="round">
              ...
              <button
                class="flex items-center justify-center w-[34px] h-[34px] bg-transparent border-none rounded-md text-text-secondary cursor-pointer transition-all duration-fast hover:bg-bg-hover hover:text-text-primary"
                @click="duplicateMock(selectedMock)" title="Duplicate Mock">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                  stroke-linecap="round" stroke-linejoin="round">
              ...
              <button
                ...
                @click="confirmDeleteMock(selectedMock)" title="Delete Mock">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                  stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex flex-col gap-4">
              ...
              <div class="grid grid-cols-3 gap-3">
                <div class="p-3.5 bg-bg-secondary border border-border-default rounded-lg">
                  <div class="text-[10px] font-semibold text-text-muted uppercase tracking-wide mb-1.5">Status Code
                  </div>
                  <div
                    :class="['text-base font-semibold', selectedMock.status >= 200 && selectedMock.status < 300 ? 'text-accent-green' : 'text-accent-yellow']">
                    {{ selectedMock.status }}
                  </div>
                </div>
                <div class="p-3.5 bg-bg-secondary border border-border-default rounded-lg">
                ...
                <div class="p-3.5 bg-bg-secondary border border-border-default rounded-lg">
                  <div class="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      ...
                      @click="openSettings" title="Configure Security Settings">
                  ...
                  <div class="flex items-center gap-2 text-[13px]">
                    <button @click="toggleSecure(selectedMock)" :class="[
              ...
              <!-- Response Preview -->
              <div
                class="flex-1 flex flex-col bg-bg-secondary border border-border-default rounded-lg min-h-[200px] overflow-hidden">
                <div
                  class="flex items-center justify-between py-2.5 px-3.5 border-b border-border-default flex-shrink-0">
                ...
                <div class="flex-1 p-3.5 overflow-auto">
                  <pre
                    class="font-mono text-xs leading-normal text-text-primary m-0 whitespace-pre-wrap break-words">{{ JSON.stringify(selectedMock.response, null, 2) }}</pre>
                </div>
              </div>
            </div>
            ...
            <div class="flex flex-col xl:order-none order-first xl:max-h-none max-h-[300px]">
              <div class="flex flex-col bg-bg-secondary border border-border-default rounded-lg h-full overflow-hidden">
                ...
                <div class="flex border-b border-border-default">
                  <button v-for="lang in ['curl', 'javascript', 'python', 'php']" :key="lang" :class="[
                    ...
                    {{ lang === 'curl' ? 'cURL' : lang === 'javascript' ? 'JavaScript' : lang === 'python' ? 'Python' :
                      'PHP' }}
                  </button>
                </div>
                <div class="flex-1 p-3.5 overflow-auto bg-bg-tertiary">
                  <pre
                    class="font-mono text-[11px] leading-normal text-text-primary m-0 whitespace-pre-wrap break-words">{{ codeSnippets[snippetLang] }}</pre>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-bg-secondary border border-border-default rounded-lg overflow-hidden">
            <div class="flex items-center justify-between py-3 px-4 border-b border-border-default">
              ...
              <div class="flex items-center gap-3">
                ...
                <button class="btn btn-primary" @click="sendTestRequest" :disabled="tryItLoading">
                  <svg v-if="tryItLoading" class="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none"
            ...
            <div class="p-4 min-h-[120px]">
              <div v-if="!tryItResponse && !tryItError"
                class="flex flex-col items-center justify-center gap-3 py-6 text-text-muted text-center">
              ...
              <div v-else class="flex flex-col gap-3">
                ...
                <div class="bg-bg-tertiary rounded-md p-3.5 max-h-[200px] overflow-auto">
                  <pre
                    class="font-mono text-xs leading-normal text-text-primary m-0 whitespace-pre-wrap break-words">{{ tryItError || JSON.stringify(tryItResponse, null, 2) }}</pre>
                </div>
              </div>
            </div>
          </div>
    ...
    <Modal :show="showSettingsModal" title="Settings" @close="showSettingsModal = false">
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Global Bearer Token
          Secret</label>
        <input v-model="settingsForm.bearerToken" type="text" placeholder="my-secret-token"
    ...
    <Modal :show="showResourceModal" title="Create Full Resource" @close="showResourceModal = false">
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Resource Name
          (plural)</label>
        <input v-model="resourceForm.name" placeholder="users"
          class="w-full py-2.5 px-3 bg-bg-input border border-border-default rounded-md text-text-primary text-sm focus:outline-none focus:border-accent-blue focus:shadow-[0_0_0_2px_rgba(0,122,255,0.2)]" />
      </div>
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Collection</label>
        <div class="relative">
          <div class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded pointer-events-none"
            :style="{ backgroundColor: getCollectionColor(resourceForm.collection) }"></div>
          <select v-model="resourceForm.collection"
      ...
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Base Path
          Prefix</label>
        <input v-model="resourceForm.basePath"
          class="w-full py-2.5 px-3 bg-bg-input border border-border-default rounded-md text-text-primary text-sm focus:outline-none focus:border-accent-blue focus:shadow-[0_0_0_2px_rgba(0,122,255,0.2)]" />
    ...
    <Modal :show="showPreviewModal" title="Response Preview" size="lg" @close="showPreviewModal = false">
      <div class="max-h-[60vh] overflow-auto bg-bg-tertiary rounded-md p-4">
        <pre class="font-mono text-xs leading-normal text-text-primary m-0 whitespace-pre-wrap break-words">{{
          previewContent }}</pre>
      </div>
    </Modal>
    ...
    <Modal :show="showDeleteConfirm" title="Delete Mock" @close="showDeleteConfirm = false">
      <p class="text-text-secondary leading-relaxed">
        ...
        <br />
        <code class="inline-block mt-2 py-1.5 px-2.5 bg-bg-tertiary rounded text-accent-orange font-mono">{{
          mockToDelete?.method }} {{ mockToDelete?.path }}</code>
      </p>
      <template #footer>
    ...
    <!-- Collection Modal -->
    <Modal :show="showCollectionModal"
      ...
      <div v-if="collectionModalMode === 'create'" class="mb-4">
        ...
        <select v-model="collectionForm.projectId"
      ...
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Collection
          Name</label>
        <input v-model="collectionForm.name" placeholder="My Project"
      ...
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Description
          (optional)</label>
        <input v-model="collectionForm.description" placeholder="API endpoints for..."
          class="w-full py-2.5 px-3 bg-bg-input border border-border-default rounded-md text-text-primary text-sm focus:outline-none focus:border-accent-blue focus:shadow-[0_0_0_2px_rgba(0,122,255,0.2)]" />
      </div>
      <div class="mb-4">
        <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-1.5">Color</label>
        <div class="flex gap-2 flex-wrap">
          <button v-for="color in collectionColors" :key="color" :class="[
          ...
          ]" :style="{ backgroundColor: color }" @click="collectionForm.color = color"></button>
        </div>
      </div>
      <template #footer>
    ...
    <Modal :show="showDeleteProjectConfirm" title="Delete Project" @close="showDeleteProjectConfirm = false">
      <p class="text-text-secondary leading-relaxed">
        ...
        <br /><br />
        <strong class="text-accent-red">Warning:</strong> This will permanently delete this project, all collections,
        folders, and requests within it.
      </p>
      <template #footer>
    ...
    <Modal :show="showDeleteGroupConfirm" title="Delete Folder" @close="showDeleteGroupConfirm = false">
      <p class="text-text-secondary leading-relaxed">
        ...
        <br />
        <code class="inline-block mt-2 py-1.5 px-2.5 bg-bg-tertiary rounded text-accent-orange font-mono">{{
          groupToDelete?.name }}</code>
        <br /><br />
        <strong class="text-accent-red">Warning:</strong> This will permanently delete <strong>{{
          groupToDelete?.mocks?.length }}</strong> mocks inside this folder.
      </p>
      <template #footer>
    ...
    <Modal :show="showDeleteRequestConfirm" title="Delete Request" @close="showDeleteRequestConfirm = false">
      <p class="text-text-secondary leading-relaxed">
        ...
        <br />
        <code class="inline-block mt-2 py-1.5 px-2.5 bg-bg-tertiary rounded text-accent-orange font-mono">{{
          requestToDelete?.method }} {{ requestToDelete?.name }}</code>
      </p>
      <template #footer>
    ...
    <!-- Rename Modal -->
    <Modal :show="showRenameModal"
      :title="`Rename ${renameType?.charAt(0)?.toUpperCase()}${renameType?.slice(1) || ''}`"
      ...
      <div class="mb-4">
        ...
        <input v-model="renameValue" :placeholder="`Enter ${renameType} name`"
          class="w-full py-2.5 px-3 bg-bg-input border border-border-default rounded-md text-text-primary text-sm focus:outline-none focus:border-accent-blue focus:shadow-[0_0_0_2px_rgba(0,122,255,0.2)]"
          @keyup.enter="renameItem" autofocus />
      </div>
      <template #footer>
    ...
    <!-- Save Request Dialog -->
    <SaveRequestDialog :show="showSaveDialog || showSaveAsDialog"
      :request="showSaveDialog ? requestToSave : requestToSaveAs" :workspaces="workspaces || []"
      :is-save-as="showSaveAsDialog" :default-collection-id="saveDialogDefaultCollectionId"
    ...
    <!-- Import Modal -->
    <ImportModal :show="showImportModal" :workspaces="workspaces || []" @close="showImportModal = false"
      @import-complete="() => { refresh(); definitionsRefreshTrigger++; }" />

    <!-- API Documentation Viewer Modal -->
    <ApiDocumentationViewer :show="showDefinitionDocs" :spec="definitionDocs?.parsedInfo || null"
      :definition-name="definitionDocs?.name || 'API Documentation'" @close="showDefinitionDocs = false" />
    ...
    <Modal :show="showGenerateModal" title="Generate Mocks" size="lg" @close="showGenerateModal = false">
      <div v-if="selectedDefinition && selectedDefinition.parsedInfo">
        <div class="mb-4">
          <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-2">Target
            Collection</label>
        ...
        <div class="mb-4">
          <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-2">Response
            Type</label>
        ...
        <div class="mb-4">
          <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-2">Response Delay
            (ms)</label>
          <input type="number" v-model.number="responseDelay"
            class="w-full py-2 px-3 bg-bg-input border border-border-default rounded-md text-text-primary text-sm focus:outline-none focus:border-accent-blue"
            min="0" />
        </div>
        ...
        <div class="mb-4">
          <div class="flex items-center justify-between mb-2">
            <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide">Select
              Endpoints</label>
            <div class="text-xs">
              <button class="text-accent-blue hover:underline mr-2"
                @click="selectedEndpoints = selectedDefinition.parsedInfo.endpoints.map(ep => `${ep.method}:${ep.path}`)">Select
                All</button>
              <button class="text-text-muted hover:underline" @click="selectedEndpoints = []">Deselect All</button>
            </div>
          </div>
          <div
            class="max-h-[300px] overflow-y-auto border border-border-default rounded-md bg-bg-tertiary divide-y divide-border-default">
        ...
        <div v-if="selectedEndpoints?.length === 1" class="mb-4">
          <label class="block text-xs font-medium text-text-secondary uppercase tracking-wide mb-2">Preview Mock
            Response</label>
          <div class="bg-bg-tertiary border border-border-default rounded-md p-3">
            <div
              v-if="getPreviewForEndpoint(parseInt(selectedEndpoints[0].split(':')[0]), selectedEndpoints[0].split(':')[1])"
              class="space-y-2">
              <div class="flex items-center gap-2">
                <span :class="[
                  ...
                  {{ getPreviewForEndpoint(parseInt(selectedEndpoints[0].split(':')[0]),
                    selectedEndpoints[0].split(':')[1])?.status }}
                </span>
                <span class="text-xs text-text-muted">{{ responseType === 'success' ? 'Success' : 'Error' }}
                  Response</span>
              </div>
              <pre
                class="text-xs font-mono text-text-primary overflow-auto max-h-[200px] whitespace-pre-wrap break-words">
            {{ JSON.stringify(getPreviewForEndpoint(parseInt(selectedEndpoints[0].split(':')[0]),
              selectedEndpoints[0].split(':')[1])?.response, null, 2) }}</pre>
            </div>
      ...
      <template #footer>
        ...
        <button class="btn btn-primary" @click="generateMocks"
    ...
    <ResponseComparison v-if="showComparison && comparisonLeft && comparisonRight" :left-response="comparisonLeft"
      :right-response="comparisonRight" @close="closeComparison" />

    <!-- Create Workspace Modal -->
    <CreateWorkspaceModal :show="showWorkspaceModal" @close="showWorkspaceModal = false"
      @created="(workspace) => { refreshWorkspaces(); selectedWorkspaceId = workspace.id; }" />

    <!-- Rename Workspace Modal -->
    <RenameWorkspaceModal :show="showRenameWorkspaceModal" :workspace="workspaceToRename"
      @close="showRenameWorkspaceModal = false; workspaceToRename = null" @renamed="refreshWorkspaces" />

    <!-- Create Project Modal -->
    <CreateProjectModal v-if="projectWorkspaceId" :show="showProjectModal" :workspace-id="projectWorkspaceId"
      :workspace-name="workspaces?.find(w => w.id === projectWorkspaceId)?.name || ''" @close="showProjectModal = false"
      @created="refreshWorkspaces()" />

    <!-- Create Folder Modal -->
    <CreateFolderModal :show="showFolderModal" :collection-id="folderCollectionId || ''"
      :collection-name="folderCollectionName" @close="handleFolderModalClose" @created="handleFolderCreated" />

    <!-- Create Request Modal -->
    <CreateRequestModal :show="showRequestModal" :folder-id="requestFolderId || ''" :folder-name="requestFolderName"
      @close="showRequestModal = false; requestFolderId = null" @created="refreshWorkspaces()" />

    <!-- Keyboard Shortcuts Help Modal -->
    <KeyboardShortcutsHelpModal :show="isHelpVisible" @close="hideHelp" />
  </div>
</template>